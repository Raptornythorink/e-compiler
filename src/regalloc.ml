...
let pick_node_with_fewer_than_n_neighbors (rig : (reg, reg Set.t) Hashtbl.t) (n: int) : reg option =
  ...
  Hashtbl.iter (fun r s -> if !found = None && Set.cardinal s < n then found := Some r) rig;
  !found
...
let pick_spilling_candidate (rig : (reg, reg Set.t) Hashtbl.t)  : reg option =
  let max_neighbors = ref 0 in
  let spilling_candidate = ref None in
  Hashtbl.iter (fun r s ->
      if Set.cardinal s > !max_neighbors then
        (max_neighbors := Set.cardinal s; spilling_candidate := Some r)
    ) rig;
  !spilling_candidate
...
let allocate (allocation: (reg, loc) Hashtbl.t) (rig: (reg, reg Set.t) Hashtbl.t)
  ...
  : int =
    ...
    | NoSpill r ->  let neighbors = Hashtbl.find_default rig r Set.empty in
                    let neighbors_colors = Set.filter_map (
                      fun r -> match Hashtbl.find_option allocation r with | Some Reg(color) -> Some color | _ -> None
                    ) neighbors in
                    let allocated_color = Set.choose (Set.diff all_colors neighbors_colors) in
                    Hashtbl.replace allocation r (Reg(allocated_color));
                    next_stack_slot
