...
*)
let rec cfg_expr_of_eexpr typ_var typ_fun varinmem (e: Elang.expr) : expr res =
  match e with
  | Elang.Ebinop (b, e1, e2) ->
    cfg_expr_of_eexpr typ_var typ_fun varinmem e1 >>= fun ee1 ->
    cfg_expr_of_eexpr typ_var typ_fun varinmem e2 >>= fun ee2 ->
    (match type_expr typ_var typ_fun e1, type_expr typ_var typ_fun e2 with
      | OK(Tptr(t1)), OK(Tptr(t2)) -> OK(Ebinop(b, ee1, ee2))
      | OK(Tptr(t1)), _ ->
        size_type t1 >>= fun sz -> OK(Ebinop(b, ee1, Ebinop(Emul, ee2, Eint(sz))))
      | _, OK(Tptr(t2)) ->
        size_type t2 >>= fun sz -> OK(Ebinop(b, Ebinop(Emul, ee1, Eint(sz)), ee2))
      | _ -> OK(Ebinop(b, ee1, ee2))
    )
  | Elang.Eunop (u, e) ->
    cfg_expr_of_eexpr typ_var typ_fun varinmem e >>= fun ee ->
    OK (Eunop (u, ee))
  ...
  | Elang.Evar v ->
      (match Hashtbl.find_option varinmem v with
        | Some(addr) ->
            type_expr typ_var typ_fun (Elang.Evar(v)) >>= fun t ->
            size_type t >>= fun sz ->
  ...
  | Elang.Ecall(f, el) ->
    List.fold_right (fun e acc ->
        acc >>= fun el ->
        cfg_expr_of_eexpr typ_var typ_fun varinmem e >>= fun e ->
        OK (e::el)
      ) el (OK []) >>= fun el ->
    OK (Ecall (f, el))
  | Elang.Eaddrof(v) -> (match v with
    | Elang.Evar(v) -> (
      ...
      | Elang.Eload(e) -> cfg_expr_of_eexpr typ_var typ_fun varinmem e
      | _ -> Error("Invalid expression")
    )
  | Elang.Eload(e) ->
      type_expr typ_var typ_fun e >>= fun t ->
      cfg_expr_of_eexpr typ_var typ_fun varinmem e >>= fun v ->
      (match t with
        | Tptr(t') -> size_type t' >>= fun sz -> OK(Eload(v, sz))
...
*)
let rec cfg_node_of_einstr typ_var typ_fun varinmem (next: int) (cfg : (int, cfg_node) Hashtbl.t)
  ...
  | Elang.Iif (c, ithen, ielse) ->
    cfg_expr_of_eexpr typ_var typ_fun varinmem c >>= fun c ->
    cfg_node_of_einstr typ_var typ_fun varinmem next cfg succ ithen >>= fun (nthen, next) ->
    cfg_node_of_einstr typ_var typ_fun varinmem next cfg succ ielse  >>= fun (nelse, next) ->
    Hashtbl.replace cfg next (Ccmp(c, nthen, nelse)); OK (next, next + 1)
  | Elang.Iwhile (c, i) ->
    cfg_expr_of_eexpr typ_var typ_fun varinmem c >>= fun c ->
    let (cmp, next) = (next, next+1) in
    cfg_node_of_einstr typ_var typ_fun varinmem next cfg cmp i >>= fun (nthen, next) ->
    Hashtbl.replace cfg cmp (Ccmp(c, nthen, succ)); OK (cmp, next + 1)
  | Elang.Iblock il ->
    List.fold_right (fun i acc ->
        acc >>= fun (succ, next) ->
        cfg_node_of_einstr typ_var typ_fun varinmem next cfg succ i
      ) il (OK (succ, next))
  | Elang.Ireturn e ->
    cfg_expr_of_eexpr typ_var typ_fun varinmem e >>= fun e ->
  ...
  | Elang.Icall(f, el) ->
    ...
    OK (next, next + 1)
  | Elang.Istore(e1, e2) ->
    cfg_expr_of_eexpr typ_var typ_fun varinmem e1 >>= fun v1 ->
    cfg_expr_of_eexpr typ_var typ_fun varinmem e2 >>= fun v2 ->
    type_expr typ_var typ_fun e1 >>= fun t ->
    size_type t >>= fun sz ->
...
let cfg_fun_of_efun prog typ_fun { funname; funargs; funbody; funvartype; funrettype; funvarinmem; funstksz } =
  ...
  cfg_node_of_einstr funvartype typ_fun funvarinmem 1 cfg 0 funbody >>= fun (node, _) ->
...
let cfg_prog_of_eprog (ep: eprog) : cfg_fun prog res =
  ...
  assoc_map_res (fun _ -> cfg_gdef_of_edef ep typ_fun) ep
