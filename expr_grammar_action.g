...
non-terminals VARDEF TYPED_FIELDDEF FIELDDEF FIELDDEFS
non-terminals ADD_EXPRS ADD_EXPR
non-terminals MUL_EXPRS MUL_EXPR
non-terminals CMP_EXPRS CMP_EXPR
non-terminals EQ_EXPRS EQ_EXPR
non-terminals ID_INSTR ID_EXPR PAR_EXPR
...
FIELDDEFS -> { [] }
FIELDDEF -> TYPE IDENTIFIER TYPED_FIELDDEF SYM_SEMICOLON { $3 $2 $1 }
FIELDDEF -> SYM_STRUCT IDENTIFIER IDENTIFIER { Node (Tfielddef, [StringLeaf("struct " ^ (string_of_stringleaf $2)); $3]) }
TYPED_FIELDDEF -> { fun id t -> Node(Tfielddef, [t; id]) }
TYPED_FIELDDEF -> SYM_LBRACKET INTEGER SYM_RBRACKET { fun id t -> Node(Tfielddef, [StringLeaf( string_of_stringleaf t ^ "[" ^ string_of_intleaf $2 ^ "]"); id]) }
...
INSTR -> TYPE IDENTIFIER VARDEF SYM_SEMICOLON { $3 $2 $1 }
...
INSTR -> SYM_LPARENTHESIS EXPR SYM_RPARENTHESIS SYM_LBRACKET EXPR SYM_RBRACKET SYM_ASSIGN EXPR SYM_SEMICOLON { Node(Tsetindex, [$2; $5; $8]) }
VARDEF -> SYM_ASSIGN EXPR { fun id t -> Node(Tvardef, [t; id; Node(Tassignvar, [$2])]) }
VARDEF -> { fun id t -> Node(Tvardef, [t; id; NullLeaf]) }
VARDEF -> SYM_LBRACKET INTEGER SYM_RBRACKET { fun id t -> Node(Tvardef, [StringLeaf( string_of_stringleaf t ^ "[" ^ string_of_intleaf $2 ^ "]"); id; NullLeaf]) }
...
ID_INSTR -> SYM_LBRACKET EXPR SYM_RBRACKET SYM_ASSIGN EXPR SYM_SEMICOLON { fun id -> Node(Tsetindex, [id; $2; $5]) }
...
FACTOR -> CHAR { $1 }
FACTOR -> SYM_LPARENTHESIS EXPR SYM_RPARENTHESIS PAR_EXPR { $4 $2 }
...
ID_EXPR -> SYM_LBRACKET EXPR SYM_RBRACKET { fun id -> Node(Tgetindex, [id; $2]) }
ID_EXPR -> { fun id -> id }
PAR_EXPR -> { fun e -> e }
PAR_EXPR -> SYM_LBRACKET EXPR SYM_RBRACKET { fun e -> Node(Tgetindex, [e; $2]) }
