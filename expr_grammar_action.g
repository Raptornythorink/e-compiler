...
axiom S
{
  ...
  let resolve_associativity (term:tree) (other: (tag*tree)list) =
    List.fold_left (fun acc (tag_v,t) -> Node(tag_v,[acc;t]) ) term other
}
...
ELSE -> SYM_ELSE LINSTRS {Node(Tblock,[$2])}
ELSE -> {NullLeaf}
...
MUL_EXPR -> FACTOR {Node(Tassignvar, [$1])}
FACTOR -> INTEGER { $1 }
FACTOR -> IDENTIFIER { $1 }
